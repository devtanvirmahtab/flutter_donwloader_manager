...
class _MyHomePageState extends State<MyHomePage> {
            ...
            return DownloadListItem(
              ...
              onActionTap: (task) {
                if (task.status == DownloadTaskStatus.undefined) {
                  ...
                  if (kDebugMode) {
                    print("Delete Task aurnab - ${task.name}");
                  }
                  _delete(task);
                  if (kDebugMode) {
                    print("Deleted Task aurnab - ${task.name}");
                  }
                } else if (task.status == DownloadTaskStatus.failed) {
                  if (kDebugMode) {
                    print("Fail Task aurnab - ${task.name}");
                  }
                  _retryDownload(task);
                  if (kDebugMode) {
                    print("Failed Task aurnab - ${task.name}");
                  }
                }
              },
              onCancel: _delete,
  ...
  Future<String?> _getSavedDir() async {
    ...
    print("externalStorageDirPath ${directory?.path}");
